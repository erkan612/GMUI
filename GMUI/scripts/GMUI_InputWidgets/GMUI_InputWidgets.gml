...
function gmui_slider_label(value, label, min_val, max_val, width = 200) {
	value = gmui_slider(value, min_val, max_val, width);
	gmui_sameline();
	gmui_text(label);
	return value;
}
...
function gmui_textbox_label(text, label, placeholder = "", width = 200, is_password = false, font = undefined) {
	text = gmui_textbox(text, placeholder, width, is_password, font);
	gmui_sameline();
	gmui_text(label);
	return text;
};
...
function gmui_input_float(value, step = 1, min_val = -1000000, max_val = 1000000, width = 150, color_identifier = undefined, dec = 8, font = undefined) {
	...
	if (!is_focused && !is_dragging) {
	    variable_struct_set(container, state_prefix + "text", string_format(value, 1, dec));
	    text = string_format(value, 1, dec);
	} else {
...
function gmui_input_float_label(value, label, step = 1, min_val = -1000000, max_val = 1000000, width = 150, color_identifier = undefined, dec = 4, font = undefined) {
	value = gmui_input_float(value, step, min_val, max_val, width, color_identifier, dec, font);
	gmui_sameline();
	gmui_text(label);
	return value;
};

function gmui_input_float_disabled(value, width = 150, font = undefined) {
    var gmui = global.gmui;
    var style = gmui.style;
    var widget = gmui_begin_widget("input_float");
...
function gmui_input_float_buttons(value, label = "", step = 1, val_min = -1000000, val_max = 1000000, color_identifier = -1, dec = 4, total_width = 240, font_label = undefined, font_input = undefined, font_button = undefined) {
	var new_value = value;
	
	var button_height = global.gmui.style.button_min_height;
	var target_width = total_width - global.gmui.style.element_spacing_h * 2;
	var input_target_width = target_width - button_height * 2;
	var input_width = input_target_width;
	var button_width = button_height;
	
	new_value = gmui_input_float(new_value, step, val_min, val_max, input_width, color_identifier, dec, font_input);
	gmui_sameline();
	if (gmui_button_minus(button_width, button_height, font_button)) {
		new_value -= step / 10;
	}
	gmui_sameline();
	if (gmui_button_plus(button_width, button_height, font_button)) {
		new_value += step / 10;
	}
...
function gmui_input_int(value, step = 1, min_val = -1000000, max_val = 1000000, width = 150, color_identifier = undefined, font = undefined) {
    var gmui = global.gmui;
    var style = gmui.style;
...
function gmui_input_int_label(value, label, step = 1, min_val = -1000000, max_val = 1000000, width = 150, color_identifier = undefined, font = undefined) {
	value = gmui_input_int(value, step, min_val, max_val, width, color_identifier, font);
	gmui_sameline();
	gmui_text(label);
	return value;
};

function gmui_input_int_disabled(value, width = 150, font = undefined) {
    var gmui = global.gmui;
    var style = gmui.style;
    var widget = gmui_begin_widget("input_int");
...
function gmui_input_int_buttons(value, label = "", step = 1, val_min = -1000000, val_max = 1000000, color_identifier = -1, total_width = 240, font_label = undefined, font_input = undefined, font_button = undefined) {
	var new_value = value;
	...
	new_value = gmui_input_int(new_value, step, val_min, val_max, input_width, color_identifier, font_input);
	gmui_sameline();
	if (gmui_button_minus(button_width, button_height, font_button)) {
		new_value -= step;
	}
...
function gmui_scrollbar_vertical(value, min_val, max_val, length, thickness = undefined) {
    ...
    var screen_x = offset[0] + widget.x;
    var screen_y = offset[1] + widget.y;
	hovered = point_in_rectangle(
        gmui.input.m_x, gmui.input.m_y,
        screen_x, screen_y,
        screen_x + track_width, screen_y + track_height
            ...
            if (group != "" && ds_map_exists(tab_registry, group)) {
                ...
                for (var k = 0; k < array_length(bar_names); k++) {
                    ...
                    if (point_in_rectangle(gmui.input.m_x, gmui.input.m_y, r[0], r[1], r[2], r[3])) {
						...
						gmui_container_animation_detected(tabs_container);
