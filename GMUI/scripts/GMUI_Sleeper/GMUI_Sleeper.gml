...
function gmui_begin_sleeper(name) {
	...
	if (begin_result) {
		gmui_style_push("container_padding_h", 1);
		gmui_style_push("container_padding_v", 1);
		gmui_cursor_set(1, 1);
		return true;
	}
	else {
...
function gmui_end_sleeper() {
	...
	if (container.state[? "_sleeper_auto_calc_width"] || container.state[? "_sleeper_auto_calc_height"]) {
		var new_width = container.context.cursor_x - global.gmui.style.element_spacing_h;
		var new_height = container.context.cursor_y - global.gmui.style.element_spacing_v;
