...
// pie chart
function gmui_plot_pie(values, labels, count, width = -1, height = 200, font = undefined) {
    var gmui = global.gmui;
    var style = gmui.style;
...
// donut chart
function gmui_plot_donut(values, labels, count, width = -1, height = 200, donut_ratio = 0.5) {
    var style = global.gmui.style;
    var saved_ratio = style.plot_pie_donut_ratio;
    style.plot_pie_donut_ratio = donut_ratio;
    var result = gmui_plot_pie(values, labels, count, width, height);
    style.plot_pie_donut_ratio = saved_ratio;
    return result;
};

// exploded pie chart
function gmui_plot_pie_exploded(values, labels, count, width = -1, height = 200, explode_all = false) {
    var gmui = global.gmui;
    var style = gmui.style;
    var widget = gmui_begin_widget("plot_pie");
    var container = widget.container;
    ...
    if (gmui_widget_is_visible(widget)) {
        ...
        var cx = widget.x + plot_width / 2;
        var cy = widget.y + plot_height / 2;
        var radius = min(plot_width, plot_height) / 2 - 20;
        var inner_radius = radius * style.plot_pie_donut_ratio;
